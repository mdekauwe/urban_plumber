...
CONTAINS
  ...
  SUBROUTINE radiation( ssnow, veg, air, met, rad, canopy )
    ...
    ! urban exp. STARTS HERE
    ! 2nd Feb 2020

    !new stuff to prescribe the net albedo
    !calculate the fraction of fsd absorbed by each of the three terms
    !direct, diffuse and combined to soil - then rescale to ensure that
    !correct total and correct ratio is retained.
    !coopting existing dummy variables - dummy, dummy2, flwv and flpwb
    dummy(:) = 0.0
    dummy2(:) = 0.0
    flwv(:) = 0.0
    flpwb(:) = 0.0
    ...
    WHERE (mask)
         ...
         flwv = rad%fbeam(:,1) * ( 1. - rad%reffbm(:,1) ) *                       &
                      ...
                      ( 1. - rad%fbeam(:,1) ) * ( 1. - rad%reffdf(:,1) ) *        &
                      EXP( -min(rad%extkdm(:,1) * canopy%vlaiw,20.) )
         flpwb = rad%fbeam(:,2) * ( 1. - rad%reffbm(:,2) )                        &
                      * rad%cexpkbm(:,2) + ( 1. - rad%fbeam(:,2) ) *              &
                      ( 1. - rad%reffdf(:,2) ) * rad%cexpkdm(:,2)
