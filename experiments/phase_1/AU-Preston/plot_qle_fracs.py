...
def main(site, fname, plot_fname=None):
    ...
    axes = [ax1]
    vars = ["Qle_tree", "Qle_grass", "Qle_bare"]
...
def read_cable_file(fname, type=None):
    ...
    df = pd.DataFrame(f.variables['Qle'][:,0,0,0], columns=['Qle_tree'])
    df['Qle_grass'] = f.variables['Qle'][:,1,0,0]
    df['Qle_bare'] = f.variables['Qle'][:,2,0,0]
...
def resample_timestep(df, type=None):
    ...
    if type == "CABLE":
        ...
        method = {'Qle_tree':'mean', 'Qle_grass':'mean', "Qle_bare":"mean"}
    elif type == "FLUX":
        # umol/m2/s -> g/C/60min
